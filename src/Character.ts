...
export default class Character implements Fighter {
  ...
  public receiveDamage(attackPoints: number): number {
    ...
    const LifeP = damage > 0 ? this._lifePoints - damage : this._lifePoints - 1;
    this._lifePoints = LifeP > 0 ? LifeP : -1;
    return this._lifePoints;
  }
  ...
  public levelUp(): void {
    ...
    const exceedMaxLife = newMaxLife > this._race.maxLifePoints;
    this._maxLifePoints = exceedMaxLife ? this._race.maxLifePoints : newMaxLife;
    this._strength += getRandomInt(1, 10);
    this._dexterity += getRandomInt(1, 10);
    this._defense += getRandomInt(1, 10);
